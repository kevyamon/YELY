...
// FOOTER INTELLIGENT COMPACT - Bouton de commande reactif, forfaits compacts et modale d'info
...
import PackageInfoModal from '../ride/PackageInfoModal';
...
const SmartFooter = ({
  ...
  const buttonText = !isUserInZone ? 'Zone non couverte' : 'Commander un Yely';
    ...
    <View style={[styles.container, { paddingBottom }]}>
      {isRider ? (
        <>
            ...
            <View style={styles.estimationWrapper}>
              ...
              >
                <View style={styles.buttonContent}>
                  {!isUserInZone && (
                    <Ionicons name="warning-outline" size={20} color={THEME.COLORS.danger} style={styles.buttonIcon} />
                  )}
                  <Text style={[
          ...
          <PackageInfoModal 
            visible={!!selectedInfoVehicle}
            onClose={() => setSelectedInfoVehicle(null)}
            vehicle={selectedInfoVehicle}
          />
        </>
      ) : (
        /* UI CHAUFFEUR */
              ...
              {isBlocked ? (
                <>
                  <Text style={styles.offlineStatusText}>
                    {isBlockedByVerification ? 'Profil en attente de validation admin' : 'Abonnement requis pour recevoir des courses'}
                  </Text>
                  <TouchableOpacity 
                    ...
                    activeOpacity={0.8}
                  >
                    <Ionicons name={isBlockedByVerification ? 'shield-outline' : 'card-outline'} size={18} color="#FFFFFF" style={{ marginRight: 8 }} />
                    <Text style={[styles.onlinePrimaryButtonText, { color: '#FFFFFF' }]}>
                      {isBlockedByVerification ? "EN ATTENTE D'ACTIVATION" : 'PAYER MON ABONNEMENT'}
                    </Text>
                  </TouchableOpacity>
                </>
              ) : (
...
const styles = StyleSheet.create({
  ...
  confirmButtonText: { color: '#121418', fontSize: 15, fontWeight: '900', letterSpacing: 0.4 },
  ...
  statBox: { flex: 1, borderRadius: 12, paddingVertical: 14, alignItems: 'center', backgroundColor: THEME.COLORS.glassSurface, borderColor: THEME.COLORS.border, borderWidth: 1 },
  ...
  offlineTextButton: { alignSelf: 'center', flexDirection: 'row', alignItems: 'center', paddingVertical: 10, marginTop: 10 },
  offlineTextButtonText: { color: THEME.COLORS.danger, fontSize: 13, fontWeight: 'bold' },
  offlineWrapper: { alignItems: 'center', width: '100%', paddingVertical: 4 },
  offlineStatusText: { color: THEME.COLORS.textTertiary, fontSize: 13, fontStyle: 'italic', marginBottom: 10 },
  onlinePrimaryButton: {
    backgroundColor: THEME.COLORS.champagneGold,
    flexDirection: 'row',
    alignItems: 'center',
  ...
  onlinePrimaryButtonText: { color: '#121418', fontSize: 15, fontWeight: '900', letterSpacing: 0.4 },
  buttonBlocked: { backgroundColor: THEME.COLORS.danger || '#E74C3C', shadowColor: THEME.COLORS.danger || '#E74C3C' },
});
